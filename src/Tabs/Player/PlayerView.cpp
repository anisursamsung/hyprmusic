...
std::string formatAudioQuality(struct mpd_status *status) {
  if (!status)
    return "";
  ...
  if (af) {
    ...
    char fmtBuf[128];
    snprintf(fmtBuf, sizeof(fmtBuf), "%.1f kHz  •  %d-bit  •  %s", sampleRateKHz,
             af->bits, (af->channels == 2 ? "Stereo" : "Mono"));
    qualityStr = fmtBuf;
  }
  if (kbitRate > 0) {
    if (!qualityStr.empty())
...
struct ParsedTrackTags {
  ...
  std::string yearGenre;
  std::string trackDisc;
};

ParsedTrackTags parseTrackTags(struct mpd_song *song,
                              const std::string &fallbackTitle) {
  ParsedTrackTags tags;
...
void PlayerView::rebuildUI(CSharedPointer<CRectangleElement> wrapper,
  ...
  m_infoBtnContainer =
      CRectangleBuilder::begin()
          ...
          ->borderColor([palette] {
            return palette
                       ? palette->m_colors.text.mix(
                             palette->m_colors.background, 0.85)
  ...
  m_infoBtnContainer->setMouseButton(
  ...
  std::string btnIcon = Components::IconProvider::getIcon(
      m_detailsVisible ? Components::IconType::CHEVRON_UP
                       : Components::IconType::CHEVRON_DOWN);
  ...
  m_infoBtnContainer->setAbsolutePosition(
      Hyprutils::Math::Vector2D(0.0, 16.0));
  ...
  m_detailsCard =
      CRectangleBuilder::begin()
          ...
          ->size(CDynamicSize(CDynamicSize::HT_SIZE_PERCENT,
                              CDynamicSize::HT_SIZE_AUTO, {0.60F, 1.0F}))
          ->commence();
      ...
      CTextBuilder::begin()
          ...
          ->color([palette] {
            return palette ? palette->m_colors.text.mix(
                                 palette->m_colors.alternateBase, 0.20)
  ...
  m_yearGenreText =
      CTextBuilder::begin()
          ...
          ->color([palette] {
            return palette ? palette->m_colors.text.mix(
                                 palette->m_colors.alternateBase, 0.40)
...
void PlayerView::updateTrackInfo(const std::string &trackText,
                                bool hasActiveTrack, unsigned elapsed,
                                unsigned total, const std::string &songUri) {
  if (!hasActiveTrack || songUri.empty()) {
    ...
    if (m_titleText)
      m_titleText->rebuild()->text("")->commence();
    if (m_artistAlbumText)
      m_artistAlbumText->rebuild()->text("")->commence();
    if (m_yearGenreText)
      m_yearGenreText->rebuild()->text("")->commence();
    if (m_qualityText)
      m_qualityText->rebuild()->text("")->commence();
    if (m_trackDiscText)
      m_trackDiscText->rebuild()->text("")->commence();
    if (m_timeText)
      m_timeText->rebuild()->text("")->commence();
  ...
  // 1. Time string
  std::string timeStr = Utils::formatTime(elapsed) + " / " + Utils::formatTime(total);
  if (m_timeText) {
    m_timeText->rebuild()->text(std::string(timeStr))->commence();
  }

  // 2. Details card background color
  auto palette = m_ctx.palette;
  if (m_detailsCard) {
    m_detailsCard->rebuild()
        ->color([palette] {
  ...
  m_ctx.runMpdCommand([this, songUri, trackText](struct mpd_connection *conn) {
    ...
    if (conn) {
      ...
      tags = parseTrackTags(song, trackText);
      if (song) {
        mpd_song_free(song);
      }

      struct mpd_status *status = mpd_run_status(conn);
      qualityStr = formatAudioQuality(status);
      if (status) {
    ...
    if (m_titleText)
      m_titleText->rebuild()->text(std::string(tags.title))->commence();
    if (m_artistAlbumText)
      m_artistAlbumText->rebuild()->text(std::string(tags.artistAlbum))->commence();
    if (m_yearGenreText)
      m_yearGenreText->rebuild()->text(std::string(tags.yearGenre))->commence();
    if (m_qualityText)
      m_qualityText->rebuild()->text(std::string(qualityStr))->commence();
    if (m_trackDiscText)
      m_trackDiscText->rebuild()->text(std::string(tags.trackDisc))->commence();

    // Check artwork updates
    if (m_lastSongUri != songUri || m_currentArtPath.empty() ||
        m_currentArtPath == Utils::getDefaultArtworkPath()) {
