...
// --- Helper: Action Menu for Playlist (Play, Queue, Rename, Delete) ---

void showPlaylistContextMenu(const PlaylistsViewContext &ctx,
                             const std::string &plName,
                             std::function<void()> onDeleted,
                             std::function<void()> onPlaylistQueued) {
  Dialogs::ActionMenuContext menuCtx{
      .options = {Components::IconProvider::getIcon(Components::IconType::PLAY) +
                  ...
                  Components::IconProvider::getIcon(
                      ...
                      " Add to Queue",
                  Components::IconProvider::getIcon(Components::IconType::EDIT) +
                      " Rename",
                  Components::IconProvider::getIcon(
                      Components::IconType::DELETE) +
                      " Delete"},
      .onSelect =
          [ctx, plName, onDeleted, onPlaylistQueued](size_t idx, const std::string &) {
            if (idx == 0) { // ▶ Play
              ...
              if (ctx.showNotification)
                ctx.showNotification("Playing " + plName);
              if (ctx.updateStatus)
                ctx.updateStatus();
              ...
              if (ctx.dialogCoordinator)
                ctx.dialogCoordinator->showRenameDialog(plName);
              else if (ctx.showRenameDialog)
                ctx.showRenameDialog(plName);
            } else if (idx == 3) { // 🗑️ Delete
              if (ctx.dialogCoordinator) {
                ctx.dialogCoordinator->showDeletePlaylistDialog(plName, onDeleted);
              } else if (ctx.showDeletePlaylistDialog) {
                ctx.showDeletePlaylistDialog(plName, onDeleted);
              } else {
                ctx.runMpdCommand([plName](struct mpd_connection *conn) {
                ...
                if (ctx.showNotification)
                  ctx.showNotification("Deleted " + plName);
                if (onDeleted)
                  onDeleted();
              }
            }
...
// --- Helper: Render a Playlist Card for the Grid ---

CSharedPointer<CRectangleElement> renderPlaylistCard(
    const PlaylistsViewContext &ctx, const std::string &plName, int trackCount,
    bool isSelected, double actualSize, int rounding,
    std::function<void()> onCardClick, std::function<void()> onDeleted,
    std::function<void()> onPlaylistQueued) {
  ...
  card->setMouseButton(
      [onCardClick](Input::eMouseButton button, bool down) {
        if (button == Input::MOUSE_BUTTON_LEFT && !down && onCardClick) {
          onCardClick();
        }
      });
  ...
  auto actionBtn =
      CButtonBuilder::begin()
          ...
          ->fontSize(CFontSize(CFontSize::HT_FONT_TEXT))
          ->onMainClick([ctx, plName, onDeleted,
                         onPlaylistQueued](CSharedPointer<CButtonElement>) {
            showPlaylistContextMenu(ctx, plName, onDeleted, onPlaylistQueued);
          })
          ->size(CDynamicSize(CDynamicSize::HT_SIZE_ABSOLUTE,
                              CDynamicSize::HT_SIZE_ABSOLUTE, {28.0F, 24.0F}))
          ->commence();
...
// --- Helper: Song Title / Artist Resolver ---

PlaylistSongItem parseSongMetadata(struct mpd_song *s, int songPos,
  ...
  return {songPos, uri ? uri : "", displayTitle, displayArtist};
}
...
void PlaylistsView::layoutPlaylists() {
  ...
  if (m_currentPlaylists.empty()) {
    ...
    for (size_t i = 0; i < m_currentPlaylists.size(); ++i) {
      ...
      auto card = renderPlaylistCard(
          m_ctx, plName, trackCount, isSelected, actualSize, rounding,
          [this, plName]() {
          ...
          [this, plName]() { addPlaylistToQueue(plName); });
  ...
  if (conn && mpd_send_list_playlist_meta(conn, plName.c_str())) {
    ...
    while ((s = mpd_recv_song(conn)) != NULL) {
      playlistSongs.push_back(
          parseSongMetadata(s, songPos++, m_ctx.ytDlpService));
      mpd_song_free(s);
    }
    mpd_response_finish(conn);
          ...
          UI::Components::SongCardConfig{
              ...
              .onActionClick =
                  [this, plName, currentPos, songUriStr] {
                            ...
                            [this, plName, currentPos,
                             songUriStr](size_t idx, const std::string &) {
                              if (idx == 0) {
                                ...
                                if (!songUriStr.empty()) {
                                  if (m_ctx.dialogCoordinator)
                                    m_ctx.dialogCoordinator->showPlaylistSelectionDialog(
                                        songUriStr, m_selectedPlaylist);
                                  else if (m_ctx.showPlaylistSelectionDialog)
                    ...
                    if (m_ctx.dialogCoordinator)
                      m_ctx.dialogCoordinator->showActionMenu(menuCtx);
                    else
                      Dialogs::showActionMenuDialog(menuCtx);
                  }});
...
void PlaylistsView::rebuildUI(CSharedPointer<CRectangleElement> wrapper,
  ...
  if (!m_detailedView || m_selectedPlaylist.empty()) {
    ...
    auto backBtn =
        CButtonBuilder::begin()
            ->label(Components::IconProvider::getIcon(Components::IconType::BACK))
            ->alignText(HT_FONT_ALIGN_CENTER)
            ->fontFamily(Components::IconProvider::getCustomFontFamily())
            ->fontSize(CFontSize(CFontSize::HT_FONT_ABSOLUTE, 18.0f))
            ->onMainClick([this](CSharedPointer<CButtonElement>) {
              m_detailedView = false;
              m_selectedPlaylist = "";
              m_ctx.runMpdCommand([this](struct mpd_connection *conn) {
