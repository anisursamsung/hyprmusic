#include "RainbowSpectrumVisualization.hpp"
...
CSharedPointer<IElement> RainbowSpectrumVisualization::build(CSharedPointer<CPalette> palette) {
  (void)palette;
...
bool RainbowSpectrumVisualization::update(const std::vector<float>& spectrum) {
  if (m_bars.empty() || spectrum.empty()) return false;

  int binsPerBar = std::max(1, static_cast<int>(spectrum.size()) / RAINBOW_BAR_COUNT);
  bool updated = false;
