/*
 * MiniArtworkCard Layout Nesting Hierarchy:
 *
...
MiniArtworkCard::MiniArtworkCard(const MiniArtworkCardConfig &cfg) : m_cfg(cfg) {}

CSharedPointer<CRectangleElement> MiniArtworkCard::build() {
    ...
    std::string titleStr = m_cfg.title.empty() ? "No currently playing songs" : m_cfg.title;
...
void MiniArtworkCard::updateImage(const std::string &imagePath) {
...
void MiniArtworkCard::updateInfo(const std::string &title, const std::string &subtitle) {
    m_cfg.title = title;
